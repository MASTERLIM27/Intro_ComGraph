...
def bspline_interp(p0, p1, p2, p3, t):
    t2 = t * t
    t3 = t2 * t
    ...
    x = int(b0 * p0[0] + b1 * p1[0] + b2 * p2[0] + b3 * p3[0])
    y = int(b0 * p0[1] + b1 * p1[1] + b2 * p2[1] + b3 * p3[1])
    return (x, y)

def drawCubicBSplineCurve(points, color, thick, steps_per_segment=50):
    if len(points) < 4:
        return
    ...
    for i in range(len(points) - 3):
        ...
        prev = bspline_interp(p0, p1, p2, p3, 0)
        for step in range(1, steps_per_segment + 1):
            t = step / steps_per_segment
            curr = bspline_interp(p0, p1, p2, p3, t)
            pygame.draw.line(screen, color, prev, curr, thick)
            prev = curr
...
while not done:   
    ...
    if len(pts) >= 4:
        ...
        drawCubicBSplineCurve(pts, BLUE, 2, 50)
